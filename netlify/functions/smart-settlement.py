...
class AdvancedSettlementCalculator:
    ...
    def calculate_optimal_settlement(self, balances: Dict[int, float], members: List[Dict]) -> Dict:
        ...
        for algo_name, algo_func in algorithms:
            try:
                cloned_people = [
                    Person(id=p.id, name=p.name, balance=p.balance)
                    for p in people
                ]
                result = algo_func(cloned_people)
        ...
        if best_result is None:
            ...
            best_result = self._simple_greedy_algorithm(cloned_people)
    ...
    def _greedy_heap_algorithm(self, people: List[Person]) -> List[Dict]:
        ...
        for person in people:
            if person.balance > 0.01:
                heapq.heappush(creditors, (-person.balance, person.id, person))
            elif person.balance < -0.01:
                heapq.heappush(debtors, (person.balance, person.id, person))  # Already negative
        
        while creditors and debtors:
            # Get the largest creditor and debtor
            creditor_balance, _, creditor = heapq.heappop(creditors)
            creditor_balance = -creditor_balance  # Convert back to positive

            debtor_balance, _, debtor = heapq.heappop(debtors)
            ...
            if creditor.balance > 0.01:
                heapq.heappush(creditors, (-creditor.balance, creditor.id, creditor))
            if debtor.balance < -0.01:
                heapq.heappush(debtors, (debtor.balance, debtor.id, debtor))
